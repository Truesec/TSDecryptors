...
{
    public class ChaosDecryptor : DecryptorBase<string, byte[]>
    {
        ...
        public ChaosDecryptor(IDecryptorCallback decryptorCallback, ILogger logger, IDecryptorChecker decryptorChecker)
        ...
        protected override void FindFiles(string targetDir, string targetExt, bool recursive = true)
        {
            try
            {
                ...
                {
                    if (decryptorChecker.CheckFile(file) == Malware.Chaos) {
                        decryptorCallback.FileProgession(file, FileStatus.Waiting);
                        ReadFirstEncryptedBlock(file);
                    } else {
                        decryptorCallback.FileProgession(file, FileStatus.FileIsNotEncryptedMaybeOverwritten);
                    }
                }
            }
            catch (System.Exception excpt)
            {
                logger?.Log("ChaosDecryptor.FindFiles: "+ excpt.Message);
            }
        }
        ...
        private void ReadFirstEncryptedBlock(string filename)
        {
            ...
            try
            {
                ...
                int RSAMsgLength = inputString.IndexOf('>', 150, 400) + 1;
                if(RSAMsgLength < 20) {
                    logger?.Log("ChaosDecryptor.ReadFirstEncryptedBlock: Unexpected format on file " + filename);
                }
                // Base64 decode the AES ciphertext
                string cipherstring = inputString.Substring(RSAMsgLength, inputString.Length-RSAMsgLength);
                byte[] b64decoded = Convert.FromBase64String(cipherstring);
                for(int i=0;i<16; i++) {
                    data[i] = b64decoded[i];
                }
            }
            ...
            {
                logger?.Log("ChaosDecryptor.ReadFirstEncryptedBlock: [Exception] " + excpt.Message);
            }
            filesToDecrypt[filename] = data;
        }

        // Iterate all keys/ivs in dbfile and try to decrypt every file in 'filesToDecrypt' 
        ...
        protected override void Iterate(string dbfile)
        {
            try
            {
                ...
                using (var fsSource = new FileStream(dbfile, FileMode.Open, FileAccess.Read))
                {
                    ...
                    // Iterate every key and iv in the database file
                    while (true)
                    {
                        int n1 = fsSource.Read(key, 0, keysize);
                        int n2 = fsSource.Read(iv, 0, ivsize);
                        ...
                        try
                        {
                            if(TryDecryption(key, iv) == false) {
                                break; // no files left to decrypt, simply exit
                            }
                        }
                        catch (Exception excpt)
                        {
                            logger?.Log("ChaosDecryptor.Iterate: [Exception] " + excpt.Message);
                        }

                        if (counter % seedProgression == 0)
                        {
                            decryptorCallback.Progress();
                        }

                        bytesRead += ivsize + keysize;
                        counter += 1;
                    }
                }
            ...
            {
                logger?.Log("ChaosDecryptor.Iterate: [Exception] " + ioEx.Message);
            }
        }

        private bool TryDecryption(byte[] key, byte[] iv)
        {
            ...
            foreach (KeyValuePair<string, byte[]> cryptedFile in filesToDecrypt)
            {
                ...
                if (magics.TryGetValue(bytesAsInt, out extensions))
                {
                    ...
                    if (Array.IndexOf(extensions, System.IO.Path.GetExtension(outputFile)) >= 0)
                    {
                        ...
                        DecryptChaosFile(cryptedFile.Key, outputFile, iv, key);
                        logger?.Log("ChaosDecryptor.TryDecryption: Decrypted '" + outputFile + "' (key=" + BitConverter.ToString(key) + ", iv=" + BitConverter.ToString(iv) + ")");
...
#pragma warning restore SYSLIB0022
        ...
        private static void DecryptChaosFile(string inputFile, string outputFile, byte[] iv, byte[] key)
        {
            // Decryption for Chaos and older Onyx variants 
            string inputString = File.ReadAllText(inputFile);
            // Define AES decryptor to match the Chaos AES encryptor
#pragma warning disable SYSLIB0022
            using (var AES = new RijndaelManaged())
            {
                AES.KeySize = 256;
                ...
                using (FileStream outputFileStream = new FileStream(outputFile, FileMode.Create))
                {
                    using (ICryptoTransform decryptor = AES.CreateDecryptor())
                    {
                        using (MemoryStream memoryStream = new MemoryStream())
                        {
                            // Calculate where the RSA message ends (wrapped in <EncryptedKey>)
                            int RSAMsgLength = inputString.IndexOf('>', 150, 400) + 1;
                            ...
                            memoryStream.Write(b64decoded, 0, b64decoded.Length);
                            memoryStream.Seek(0, 0);
